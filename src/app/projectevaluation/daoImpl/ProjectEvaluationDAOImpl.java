...
public class ProjectEvaluationDAOImpl implements ProjectEvaluationDAO{
	...
	@Value("${getProjectwiseCommunityBasedData}")
	String getProjectwiseCommunityBasedData;
...
//				query.setInteger("dcode",dcode); 
		...
		public List<ProjectEvaluationBean> getprojMidProjEvoluation(int dCode) {
			List<ProjectEvaluationBean> getProjMidProjEvoluation = new ArrayList<ProjectEvaluationBean>();
			String hql = getProjMidProjEvlDetails;
			Session session = sessionFactory.getCurrentSession();
			try {
				session.beginTransaction();
				SQLQuery query = session.createSQLQuery(hql);
				query.setInteger("dcode", dCode);
				query.setResultTransformer(Transformers.aliasToBean(ProjectEvaluationBean.class));
				getProjMidProjEvoluation = query.list();
				session.getTransaction().commit();
			} catch (Exception ex) {
				ex.printStackTrace();
				session.getTransaction().rollback();
		...
		public List<ProductionDetailsBean> getProjwiseCommunityBasedData(Integer dcode) {
			List<ProductionDetailsBean> list = new ArrayList<>();
			String hql = getProjectwiseCommunityBasedData;
			Session session = sessionFactory.getCurrentSession();
