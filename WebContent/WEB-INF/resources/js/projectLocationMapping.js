...
$(function(){
	...
	console.log(data);
						...
						if(btnHide){
							$('.error').html('You have already assigned location to this project. Data has been locked.!');
	...
	console.log(data);
						...
						for(var y in d2){
							if(d2[y].wcid===d[k].wcid1){
								$wsSelect+='<option value="'+d[k].wcid1+'"selected>'+d2[y].wcname+'</option>';
								}
							else
	...
	$('.error').text('You have already mapped villages with watershed committee.!');
	...
	function saveDraft(e){
		...
		$.ajax({  
            ...
            success: function(data) {
					...
					$("#successok").click(function(){
						$('#popup').modal('hide');
						window.location.href='piaPjtNotLocatiaon';
					});
                }
        });
e.preventDefault();
	...
	function completeData(e){
		...
		$.ajax({  
            ...
            success: function(data) {
					...
					if(data){
						successAlert('Project Location assigned successfully !');
					}else{
						successAlert('There is some error. Please try again !')
					}
					$("#successok").click(function(){
