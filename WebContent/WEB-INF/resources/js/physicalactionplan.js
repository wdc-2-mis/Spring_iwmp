	...
	if(Object.keys(data).length>0 && !planexist){
	 for ( var key in data) {
		if (data.hasOwnProperty(key)) {
			...
			tbodyPhysicalAnnualActionPlan.append("<tr id='tr"+(parseInt(key)+1)+"'><td>"+data[key].headname+"</td><td>"+data[key].activityname+"</td><td>"+data[key].unitname+"</td><td>"+data[key].plan+"</td><td><a href='#' data-id='"+data[key].aapid+"' class='deleteannualactionplan'>delete</a></td></tr>");
		}
			}
			$('.remarks').show();
...
//	alert('kdy '+i);
	...
	//	alert('kdy11');
		...
		$('#listActivityHeadWiseTbody').append('<tr id="tr"><td><select id="ddlHead" name="ddlHead" class="ddlHead form-control"><option value="">--Select Head--</option></select></td><td><select id="ddlActivity" name="ddlActivity" class="ddlActivity form-control"><option value="">--Select Activity--</option></select></td><td><input type="text" id="ddlUnit" name="ddlUnit" class="ddlUnit form-control" readonly /><!--<select id="ddlUnit" name="ddlUnit" class="ddlUnit form-control"><option value="">--Select Unit--</option></select>--></td><td><input type="text" id="txtTarget" name="txtTarget" class="form-control" value=""/></td><td></td></tr>');*/
            	...
            	else if(data==='activity exist')
            	{
            		successAlert("Selected activity already exist. Please select different one.<br/> Incase you want to modify the selected activity then delete it and save it again!");
            		$("#successok").click(function(){
            			$('#popup').modal('hide');
            		});  
            		$(".close").click(function(){
...
$('#viewMovement').click(function(e) {
	...
	/**************************************************** View Complete Plan Code ********************************************************* */
	$('#viewCompletePlan').click(function(e) {
	var proj=[];
	var yr=[];
	$('#loading').show();
	$.ajax({  
            url:"viewCompleteMovement",
            type: "post",  
            data: {},
            error:function(xhr,status,er){
                console.log(er);
            ...
            success: function(data) {
				console.log(data);
	$('#loading').hide();
	var tbodyCMovement = $('#tbodyCMovement');
	...
	for ( var key in data) {
		...
		if (d.hasOwnProperty(k)) {
			if(parseInt(k)===0)
			tbodyCMovement.append("<tr><td>"+key+"</td><td>"+d[k].finyr+"</td><td>"+d[k].status+"</td><td>"+d[k].currentlevel+"</td><td class='remarks'>"+d[k].remarks+"</td></tr>");
			else
			tbodyCMovement.append("<tr><td></td><td>"+d[k].finyr+"</td><td>"+d[k].status+"</td><td>"+d[k].currentlevel+"</td><td class='remarks'>"+d[k].remarks+"</td></tr>");
		}
		}
		...
		tbodyCMovement.append("<tr><td colspan='5' class='text-center'>Data not found !</td></tr>");
	}
	}
	});
	});
...
if($remarks==="" && $action==='R' ){
	...
	$('#loading').hide();
						...
						if(data==='state'){
							if($action==='R')
							successAlert('Data of Annual Action Plan is being rejected. And backforward to District');
							else
						...
						if($action==='R')
							successAlert('Data of Annual Action Plan is being rejected. And backforward to pia.');
							else
