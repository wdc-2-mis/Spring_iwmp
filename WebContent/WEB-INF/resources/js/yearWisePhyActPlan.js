	...
	$(document).on('change', '#state', function(e) {
		...
		$.ajax({  
            ...
            success:function(data) {
						...
						$ddlDistrict.empty();
      					if($selectedDist == 0){
							$ddlDistrict.append('<option value="0" selected> --All-- </option>');
						}else{
		...
		$.ajax({  
            ...
            success:function(data) {
        				...
        				if($selectedProject ==0){
							$ddlProject.append('<option value="0" selected> --All-- </option>');
						}else{
		...
		$(document).on('change', '#project1', function(e) {
		e.preventDefault();
		$pCode=$('#project option:selected').val();
		$.ajax({  
            url:"getFromYearForPhysicalActionPlanReport",
            type: "post", 
			data:{pCode:$pCode}, 
            error:function(xhr,status,er){
                console.log(er);
		...
		/*************************************************** From Year Change ********************************************** */
		$(document).on('change', '#fromyear1', function(e) {
		e.preventDefault();
		$fromYear=$('#fromyear option:selected').val();
		$projId = $('#project option:selected').val();
		$.ajax({  
            url:"getToYearForPhysicalActionPlanReport",
            type: "post", 
...
//	function exportPDF(){
//	var table="";
//	var reportName = "Format T1- Physical Action Plan of a Project Periodically";
...
//$('#project option:selected').text()+" \r\nFrom: "+$('#fromyear option:selected').text()+" To: "+$('#toyear option:selected').text();
//
...
//
//	var fileName="Year-wise Physical Action Plan of a Project";
//	 
//	var reportName = "Format T1- Physical Action Plan of a Project Periodically";
...
//	    "<tr><td>From: "+$('#fromyear option:selected').text()+"</td><td>To: "+$('#toyear option:selected').text()+"</td>"+
//	    "<tr><td></td></tr></table>";
