...
<title>Report ME4- State wise Current Status of Capacity Building Activities, UGs, SHGs, FPOs, EPAs, Production System and Livelihood Activities</title>

	<script type="text/javascript">
	function exportExcel(){
		document.getCurrentStatusOther.action="downloadExcelStateWiseStatusOtherActivity";
		document.getCurrentStatusOther.method="post";
		document.getCurrentStatusOther.submit();
	}
	function downloadPDF(){
		document.getCurrentStatusOther.action="downloadStateWiseStatusOtherActivityPDF";
		document.getCurrentStatusOther.method="post";
		document.getCurrentStatusOther.submit();
	}
	</script>
<body>
<br>
<div class="offset-md-3 col-6 formheading" style="text-align:center;"  ><h5><label id="head">Report ME4- State wise Current Status of Capacity Building Activities, UGs, SHGs, FPOs, EPAs, Production System and Livelihood Activities</label> </h5></div>
<br>
<form action="downloadExcelStateWiseStatusOtherActivity" method="post" id="getCurrentStatusOther" name="getCurrentStatusOther">
			<div class="form-row">
			</div>
		</form>
		...
		<c:forEach var="list" items="${netTreatledata}">
			<c:forEach var="listUser" items="${list.value}" >
				<tr>
					...
					<td><a href = "getdistrictWiseStatusOtherActivity?id= <c:out value='${listUser.st_code}' />&stname=${listUser.st_name}"><c:out value='${listUser.st_name}' /></a></td>
