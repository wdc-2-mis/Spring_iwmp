...
<head>
<title>Report SB3-District wise Distribution of Area According to Irrigation Status, Ownership Status, Land Classification and Number of Crops as per Baseline Survey</title>
</head>

<script type="text/javascript">
function downloadPDF(stateName, stcode)
{
		document.getElementById("stateName").value=stateName;
		document.getElementById("stcode").value=stcode;
		document.getblsprpt.action="downloadblsurveyPDFDistrictWise";
		document.getblsprpt.method="post";
		document.getblsprpt.submit();
} 
function exportExcel(stateName, stcode)
{
	document.getElementById("stateName").value=stateName;
	document.getElementById("stcode").value=stcode;
	document.getblsprpt.action="downloadExcelDistrictWiseBaselineArea";
	document.getblsprpt.method="post";
	document.getblsprpt.submit();
}
	
	</script>
<form action="downloadblsurveyPDFDistrictWise" method="post" name="getblsprpt">
...
<!-- 	<tbody id="dtBasicExample"> -->
		<thead>
			<tr>
				<th align="left" colspan="10">State: &nbsp;<c:out value="${stateName}" />  </th>
			...
			<c:forEach items="${distwiseAreaDtlList}" var="project" varStatus="sno">
				<tr>
...
<%-- 								value="${project.stname}" /></a></td> --%>
					<td><a href="getProjWiseblsareasrvydtlrpt?distcode=<c:out value="${project.distcode}"/>&distname=<c:out value="${project.distname}" /> &stname=<c:out value="${stateName}" />"><c:out value="${project.distname}" /></a></td>
