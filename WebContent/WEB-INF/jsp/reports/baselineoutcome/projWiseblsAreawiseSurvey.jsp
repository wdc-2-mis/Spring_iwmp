...
<head>
<title>Report SB3-Project wise Distribution of Area According to Irrigation Status, Ownership Status, Land Classification and Number of Crops as per Baseline Survey</title>
</head>

<script type="text/javascript">
function downloadPDF(stateName, distname, distcode)
{
	
		document.getElementById("stateName").value=stateName;
		document.getElementById("distname").value=distname;
		document.getElementById("distcode").value=distcode;
		document.getblsprpt.action="downloadblsurveyPDFProjectWise";
		document.getblsprpt.method="post";
		document.getblsprpt.submit();
		
} 
	
function exportExcel(stateName, distname, distcode)
{
	
	document.getElementById("stateName").value=stateName;
	document.getElementById("distname").value=distname;
	document.getElementById("distcode").value=distcode;
	document.getblsprpt.action="downloadExcelProjectWiseBaselineArea";
	document.getblsprpt.method="post";
	document.getblsprpt.submit();
}
	
</script>
<form action="downloadblsurveyPDFProjectWise" method="post" name="getblsprpt">
...
<div class="container-fluid">
	<div class="offset-md-3 col-6 formheading" style="text-align: center;">
		<h5><label id="head1">Report SB3-District wise Distribution of Area According to Irrigation Status, Ownership Status, Land Classification and Number of Crops as per Baseline Survey</label></h5>
	</div>
	 <button name="exportExcel" id="exportExcel" onclick="exportExcel('${stateName}','${distname}','${distcode}')" class="btn btn-info">Excel</button>
	<button name="exportPDF" id="exportPDF" onclick="downloadPDF('${stateName}','${distname}','${distcode}')"	class="btn btn-info">PDF</button>
...
<!-- 	<tbody id="dtBasicExample"> -->
		<thead>
			...
			<c:forEach items="${projwiseAreaDtlList}" var="project" varStatus="sno">
				<tr>
...
<%-- 								value="${project.stname}" /></a></td> --%>
					...
					<td class="text-right"><fmt:formatNumber minFractionDigits="4"><c:out value="${project.plot_area}" /></fmt:formatNumber></td>
					<td class="text-right"><fmt:formatNumber minFractionDigits="4"><c:out value="${project.protective_irrigation}" /></fmt:formatNumber></td>
