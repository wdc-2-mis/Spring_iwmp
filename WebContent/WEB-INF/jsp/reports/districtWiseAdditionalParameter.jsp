...
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
     <title>Report O1- District wise Mandays Generated and Farmers benefitted Details </title>  
<script type= "text/javascript">
function exportExcel(stname, stcode,finyear,finYrName){
	...
	document.outcome.action="getDistWiseMandaysExcel";
	document.outcome.method="post";
	document.outcome.submit();
}
	function downloadPDF(stname, stcode,finYrName){
		document.getElementById("stname").value=stname;
		document.getElementById("stcode").value=stcode;
		document.getElementById("finYrName").value=finYrName;
		document.outcome.action="downloadDistrictAdditionalParameterPDF";
		document.outcome.method="post";
		document.outcome.submit();
	}
</script>
<body>
<br>
<form action="downloadDistrictAdditionalParameterPDF" method="post" id="outcome" name="outcome">
<div class="form-row">
...
<div class="offset-md-3 col-6 formheading" style="text-align:center;" ><h5><label id="head">Report O1- District wise Mandays Generated and Farmers benefitted Details for <c:out value ="${finYrName}"/> Financial Year</label></h5></div>
<br>
<div class ="card">
...
<%-- 	<form action="getDistWiseStatusAdditionalParameter" method="get"> --%>
...
<%-- 	</form> --%>
	...
	<button name="exportExcel" id="exportExcel" onclick="exportExcel('${stname}','${stcode}','${fyear}','${finYrName}')" class="btn btn-info">Excel</button>
	<button name="exportPDF" id="exportPDF" onclick="downloadPDF('${stname}','${stcode}','${finYrName}')" class="btn btn-info">PDF</button>    
	<p align="right"> Report as on: <%=app.util.Util.dateToString(null,"dd/MM/yyyy hh:mm aaa")%> </p>
	<table id="dtBasicExample" cellspacing="0" class="table" >
  	<thead>
  		<tr>
			<th colspan="12" class="text-left"> State Name : &nbsp; <c:out value='${stname}' /></th>
		</tr>
    	<tr>
		...
		<c:forEach var="list" items="${netTreatledata}">
			<c:forEach var="listUser" items="${list.value}" >
				<tr>
					...
					<td><a href="getProjWiseStatusAdditionalParameter?dcode=<c:out value="${listUser.dcode}"/>&fyear=<c:out value="${fyear}"/>&fyrname=<c:out value="${finYrName}"/>"><c:out value='${listUser.dist_name}' /> </a></td>
