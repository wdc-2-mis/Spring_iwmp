...
<html>
<head>
<title>Report O12 - Project and Financial Year wise Achievements on Farmer Income and Crop Area(OOMF-2).</title>
...
<script type="text/javascript">
	...
	function exportExcel(dcode, year, stName, distName, yrName) {
		document.getElementById("stName").value = stName;
		document.getElementById("distName").value = distName;
		document.getElementById("yrName").value = yrName;
		document.getElementById("dcode").value = dcode;
		document.getElementById("year").value = year;
		document.indicators.action = "getProjWiseFrmrIncmExcel";
		document.indicators.method = "post";
		document.indicators.submit();
	}
	
	function downloadPDF(dcode, year, stName, distName, yrName) {
		document.getElementById("stName").value = stName;
		document.getElementById("distName").value = distName;
		document.getElementById("yrName").value = yrName;
		document.getElementById("dcode").value = dcode;
		document.getElementById("year").value = year;
		document.indicators.action = "getProjWiseFrmrIncmPDF";
		document.indicators.method = "post";
		document.indicators.submit();
	}
...
<body>
 <br>
<div class="offset-md-3 col-6 formheading" style="text-align:center;"  ><h5>Report O12 - Project and Financial Year wise Achievements on Farmer Income and Crop Area(OOMF-2).</h5></div>
...
<button name="exportExcel" id="exportExcel" onclick="exportExcel('${dcode}','${year}','${stName}','${distName}','${yrName}')" class="btn btn-info">Excel</button>
<button name="exportPDF" id="exportPDF" onclick="downloadPDF('${dcode}','${year}','${stName}','${distName}','${yrName}')" class="btn btn-info">PDF</button>
<p align="right"> Report as on: <%=app.util.Util.dateToString(null,"dd/MM/yyyy hh:mm aaa")%> </p>
</c:if>
<table id="tblReport" cellspacing="0" class="table" width="auto">
  <thead>
	 <tr>
		<th colspan ="2" style="text-align:left; ">State: <c:out value ="${stName}"/> &nbsp; District: <c:out value="${distName}"/>  &nbsp; Financial Year: <c:out value ="${yrName}"/></th>
  ...
  <tbody>
	 ...
	 <c:if test="${finalList != null}">
		...
		<tr>		
			...
			<td class = "text-center"><c:out value='${dataV.projname}' /></td>
			<c:forEach items="${areaMap}" var="areaMap" varStatus="status">
				<c:if test ="${dataV.projid eq fn:substringBefore(areaMap.key,',')}">
     		...
     		<c:forEach items="${frmrMap}" var="frmrMap" varStatus="status">
				<c:if test ="${dataV.projid eq fn:substringBefore(frmrMap.key,',')}">
