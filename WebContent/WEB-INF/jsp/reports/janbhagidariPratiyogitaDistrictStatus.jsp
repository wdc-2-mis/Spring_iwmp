...
<html>
<title>District wise Current Status of Watersheed Janbhagidari</title>
<script type="text/javascript">
	
	function exportExcel(id, stname){
		document.getElementById("id").value=id;
		document.getElementById("stname").value=stname;
		document.getJanbhagidariStatus.action="ExceldistrictWiseCurrentStatusJanbhagidari";
		document.getJanbhagidariStatus.method="post";
		document.getJanbhagidariStatus.submit();
	}
	...
	function downloadPDF(id, stname)
	{
		document.getElementById("id").value=id;
		document.getElementById("stname").value=stname;
		document.getJanbhagidariStatus.action="districtWiseCurrentStatusJanbhagidariPDF";
		document.getJanbhagidariStatus.method="post";
		document.getJanbhagidariStatus.submit();
...
<div class ="card">
	...
	<div class="col-1" ></div>
	<div class="col-10" >
	<form action="getdistrictWiseCurrentStatusJanbhagidari" method="post" id="getJanbhagidariStatus" name="getJanbhagidariStatus">
			<div class="form-row">
	 ...
	 <button name="exportExcel" id="exportExcel" onclick="exportExcel('${stcode}','${stname}')" class="btn btn-info">Excel</button> 
	<button name="exportPDF" id="exportPDF" onclick="downloadPDF('${stcode}','${stname}')" class="btn btn-info">PDF</button>    
	<p align="right"> Report as on: <%=app.util.Util.dateToString(null,"dd/MM/yyyy hh:mm aaa")%> </p>
	<table id="dtBasicExample" cellspacing="0" class="table" >
  	<thead>
  	<tr> 
	 	<th colspan="15" style="text-align:left; ">State Name : <c:out value="${stname}" />   </th>
	</tr>
    	<tr>
      		...
      		<th rowspan="2" style="text-align:center; vertical-align: middle; width: 7%;">Total No. of Project</th>
      		<th rowspan="2" style="text-align:center; vertical-align: middle; width: 7%;">Total No. of Reported Project</th>
      		<th rowspan="2" style="text-align:center; vertical-align: middle; width: 7%;">Total No. of Not Reported Project</th>
  	...
  	<tbody>
		...
		<c:set var="count" value="1" />
						...
						<c:forEach items="${dataList}" var="data" varStatus="count">							
							<tr>
								<td><c:out value='${count.count}' /></td>
 								...
 								<td class="text-right"> <c:out value="${data.total_project}" /></td>
 								<td class="text-right"> <c:out value="${data.proj_repoted}" /></td>
 								<td class="text-right"> <c:out value="${data.proj_notrepoted}" /></td>
			...
			<tr>
				...
				<td align="right" class="table-primary" ><b><c:out value='${total_projectt}' /> </b></td>
				<td align="right" class="table-primary" ><b><c:out value='${proj_repotedt}' /> </b></td>
				<td align="right" class="table-primary" ><b><c:out value='${proj_notrepotedt}' /> </b></td>
