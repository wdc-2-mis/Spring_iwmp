...
<title>Report WM5 - Watershed Mahotsav Social Media Winner's</title>
...
$('#submitBtn').on('click', function() {
        document.mohotsavRpt.action = "wmSocialMediaWinnerReport";
...
function dataAvailableFunction() {
	...
	function exportExcel()
	{
		...
		var stName = document.getElementById("state").options[document.getElementById("state").selectedIndex].text;
	    var media = document.getElementById("mediaType").options[document.getElementById("mediaType").selectedIndex].value;
	    var mediaText = document.getElementById("mediaType").options[document.getElementById("mediaType").selectedIndex].text;
	    
	    document.getElementById("stName").value = stName;
	    document.getElementById("media").value = mediaText;
	    
		document.mohotsavRpt.action="downloadExcelSocialMediaWinnerReport";
		document.mohotsavRpt.method="post";
		document.mohotsavRpt.submit();
	}
	...
	function downloadPDF()
	{
	    ...
	    document.mohotsavRpt.action="downloadPDFSocialMediaWinnerReport";
		document.mohotsavRpt.method="post";
		document.mohotsavRpt.submit();
	}
    </script>
...
<body>
	...
	<div class="maindiv">
		...
		<div class="card shadow mt-1 p-5">
			...
			<div class="offset-md-3 col-6 formheading" style="text-align: center;">
				<h4 class="text-center text-primary mb-4 text-decoration-underline">Report WM5 - Watershed Mahotsav Social Media Winner's</h4>
			</div>
    ...
    <c:choose>
        ...
        <c:otherwise>
            <select id="mediaType" name="mediaType" class="form-select" required>
                <option value="ALL" ${selectedMediaType eq 'ALL' ? 'selected' : ''}>All</option>
                <option value="P" ${selectedMediaType eq 'P' ? 'selected' : ''}>Photo</option>
                <option value="V" ${selectedMediaType eq 'V' ? 'selected' : ''}>Video</option>
            </select>
        </c:otherwise>
    </c:choose>
</div>
						...
						<th colspan="${selectedMediaType eq 'P' ? 4 : 5}" style="text-align: center; vertical-align: middle;">
							<c:choose>
								<c:when test="${selectedMediaType eq 'P'}">List of Uploaded Photos</c:when>
								 <c:when test="${selectedMediaType eq 'ALL' || selectedMediaType eq 'A'}">
                                  List of All Uploaded Photos & Videos
                                 </c:when> 
