...
function downloadDPDF(stcd){
	document.getElementById("stcd").value=stcd;
	document.getWMProjLvlDetails.action="downloadDistPDFProjLvlProgram";
	document.getWMProjLvlDetails.method="post";
	document.getWMProjLvlDetails.submit();
}
...
$(document).on('click', '.showDistrictImage', function(e) {
	
	let distCode = $(this).data('id');
	let imgType = $(this).data('type');
	$.ajax({
		type: 'POST',
		url: "getDistImageMahotProgRpt",
		data: { distCode: distCode, imgType: imgType },
		error: function(xhr, status, er) {
			console.log(er);
		},
		success: function(data) {
...
<div class="maindiv">
    <div class="card shadow mt-1 p-5"> 
         <c:if test="${projLvlWMPrgListSize ne null}">
         <div class="offset-md-3 col-6 formheading" style="text-align: center;">
        	<h4 class="text-center text-primary mb-4"><u>Report WM2 - State Wise Project Level Watershed Mahotsav Program</u></h4>
    	</div>
    	</c:if>
    	
    	<c:if test="${distWMProjListSize ne null}">
    	<div class="offset-md-3 col-6 formheading" style="text-align: center;">
        	<h4 class="text-center text-primary mb-4"><u>Report WM2 - District Wise Project Level Watershed Mahotsav Program</u></h4>
    	</div>
    	</c:if>
        ...
        <c:if test="${projLvlWMPrgListSize ne null}">
                      ...
                      <tbody id="tbodyProjLvlProgRpt">
						<c:forEach items="${projLvlWMPrgList}" var="dt" varStatus="sno">
							<tr>
								...
								<td><a href = "distWMProjLvlProgRpt?stcd=${dt.st_code}&stName=${dt.stname}"><c:out value='${dt.stname}'/></a></td>
            ...
            <c:if test="${distWMProjListSize ne null}">
                <table class="table table-bordered table-striped" id="stWMI" >
                    <thead>
                    <tr>
                        <th style="text-align:left;" colspan = "20">State Name: ${stName}</th>
                    </tr>
                        <tr>
						<th rowspan="3" style="text-align:center; vertical-align: middle;">S.No.</th>
					...
					<tr>
                      ...
                      <tbody id="tbodyProjLvlProgRpt">
						<c:forEach items="${distWMProjList}" var="dt" varStatus="sno">
							<tr>
								<td class="text-left"><c:out value="${sno.count}" /></td>
								...
								<td class="text-end">
									<c:choose>
										...
										<c:otherwise>
											<a href="#" data-id="${dt.district}" data-type="prabhat" class="showDistrictImage" data-toggle="modal" style ="color: blue;"> <c:out value="${dt.total_prabhat_photo}" /></a>
										</c:otherwise>
									</c:choose>
								</td>
								<td class="text-end">
								...
								<td class="text-end">
									<c:choose>
										...
										<c:otherwise>
										<a href="#" data-id="${dt.district}" data-type="projectlvl" class="showDistrictImage" data-toggle="modal" style ="color: blue;">	<c:out value="${dt.total_projlvl_photo}" /></a>
										</c:otherwise>
										</c:choose>
								</td>
						...
						<c:if test="${distWMProjListSize>0}">
							<tr class="table-secondary fw-bold">
 								<td colspan="2" class="text-end"><b>Grand Total</b></td> 
								<td class="text-end"><b><c:out value="${totPPOrganized}" /></b></td>
								<td class="text-end"><b><c:out value="${totPPMale}" /></b></td>
