...
<div class="container mt-2">
  ...
  <form class="form" autocomplete="off" modelAttribute="baselinemain" autocomplete="off">
 ...
 <c:forEach items="${outHeadActivity}" var="act">
  ...
  <c:forEach items="${phyHeadActivity1}" var="phyAct">
	...
	<c:choose>
					...
					<div class="form-group col-md-4 ">
						<label for="" class="">   <c:out	value="${val.value }" /> </label>
						<input type="text" class="form-control" readonly onfocusin="decimalCheck(event)" id="${fn:replace(fn:replace(fn:replace(fn:replace(fn:replace(val.value,')',''),'(',''),'/',''),'.',''),' ','')}_${phyAct.key.headCode }_${val.key}C" name="${fn:replace(val.value,' ','')}_${phyAct.key.headCode }_${val.key}C" onfocusin="numericOnly(event)">
					</div>	
					</c:forEach>
					</c:when>
					<c:otherwise>
						<div class="form-group col-md-8 ">
    <label for="" class="">Total Existing <c:out	value="${phyAct.key.headDesc }" /></label>
    <input type="text" class="form-control" readonly onfocusin="decimalCheck(event)" id="${fn:replace(fn:replace(fn:replace(fn:replace(fn:replace(phyAct.key.headDesc,')',''),'(',''),'/',''),'.',''),' ','')}_${phyAct.key.headCode }_0C" name="${fn:replace(act.key.headDesc,' ','')}_${phyAct.key.headCode}_0" onfocusin="numericOnly(event)">
  </div>
					</c:otherwise>
				</c:choose>
				</div>
