...
<html>
<head>
<title>Report OC1- District wise details</title>

</head>

<script type="text/javascript">
function exportExcel(stcd, headtype, stname) {
	document.getElementById("headtypeh").value=headtype;
	document.getElementById("stcd").value=stcd;
	document.getElementById("stname").value=stname;
	document.rptSHGUserGroup.action="downloadExcelDistSHG";
...
function downloadPDF(stcd, headtype, stname)
{
		document.getElementById("headtypeh").value=headtype;
		document.getElementById("stcd").value=stcd;
		document.getElementById("stname").value=stname;
		document.rptSHGUserGroup.action="downloadPDFDistSHG";
		document.rptSHGUserGroup.method="post";
		document.rptSHGUserGroup.submit();
...
<div class="container-fluid">
	<div class="row">
		<div class="col text-center">
			<br />
			<h5><div class="offset-md-3 col-6 formheading" style="text-align:center;"  >Report OC1- District wise details of <c:if test="${headtype eq 'shg'}">
...
<div class="row">
	...
	<div class="col-12">
		...
		<div class="col" id="reportDiv">
			...
			<!-- <button name="exportExcel" id="exportExcel" onclick="exportExcel()" '${rpt.dist_code}', '${rpt.dist_name}'
				class="btn btn-info">Excel</button>
			<button name="exportPDF" id="exportPDF" onclick="exportPDF()"
				class="btn btn-info">PDF</button> -->
				<button name="exportExcel" id="exportExcel" onclick="exportExcel('${stcd}', '${headtype}', '${stname}')" class="btn btn-info">Excel</button>
				<button name="exportPDF" id="exportPDF" onclick="downloadPDF('${stcd}', '${headtype}', '${stname}');" class="btn btn-info">PDF</button>	
			...
			<table id="tblReport" cellspacing="0"  style="width: 100%; margin-left: auto; margin-right: auto;">
				...
				<tr>
						<th colspan="32" >State : &nbsp; <c:out value='${stname}' /> &nbsp;&nbsp;&nbsp; &nbsp;&nbsp;
					...
					<tr>
						...
						<th rowspan="3" width="10%" style="text-align: center;">District Name</th>
				...
				<c:forEach items="${rptList}" var="rpt" varStatus="status">
					<tr>
						...
						<c:if test = "${ rpt.totno eq 0}" ><td>${rpt.dist_name}</td></c:if>
						<c:if test = "${ rpt.totno ne 0}" ><td>
						<a href="#" onclick="showProjectWise('${rpt.dist_code}', '${rpt.dist_name}', '${headtype}', '${stname}');">${rpt.dist_name}</a></td></c:if>
			...
			<table id="tblReport" cellspacing="0" style="width: 60%; margin-left: auto; margin-right: auto;" >
				<thead>
					<tr>
						<th colspan="8" >State : &nbsp; <c:out value='${stname}' /> &nbsp;&nbsp;&nbsp; &nbsp;&nbsp;
						Head : &nbsp;<c:if test="${headtype eq 'shg'}"> Self Help Group(SHG) </c:if> 
						<c:if test="${headtype eq 'ug'}"> User Group </c:if></th>
					</tr>
					<tr>
				...
				<c:forEach items="${beanListGroup}" var="rpt" varStatus="status">
					<tr>
						...
						<c:if test = "${ rpt.newno eq null}" ><td>${rpt.dist_name}</td></c:if>
						<c:if test = "${ rpt.newno ne null}" ><td>
						<a href="#" onclick="showProjectWise('${rpt.dist_code}', '${rpt.dist_name}', '${headtype}', '${stname}');">${rpt.dist_name}</a></td></c:if>
