...
function showReport(e)
{
	...
	if(year==='')
	{
		...
		document.unfreezeworkstatus.action="getOmmfQuarterlyTarData";
...
function selects(ele) {
    var checkboxes = document.getElementsByName('qurtr');
...
function checkAllCheckboxes() {
    var checkboxes = document.getElementsByName('qurtr');
    var selectAllCheckbox = document.getElementById('allqurtr');
...
function completebsl(){ 
		...
		if(isAnyChecked){
			...
			{
					document.unfreezeworkstatus.action="unfreezeOmmfQuarterlyTarData";
					document.unfreezeworkstatus.method="post";
					document.unfreezeworkstatus.submit();
			}
		}else{
...
<div class="col" style="text-align:center;"><h5>Unfreeze OOMF Quarter-wise Target</h5></div>
 <form:form autocomplete="off" name="unfreezeworkstatus" id="unfreezeworkstatus"  action="unfreezeOomfQuarterlyTarget" method="get">
     ...
     <div style="width: 100%;">
		<c:if test="${not empty state and not empty district and not empty project and not empty year}">
            	...
            	<thead>
              		...
              		<tr>
              				<th class="displ" align="center"><input type="checkbox" id="allqurtr" onchange="selects(this);" <c:out value ="${qAllStatus}"/> >&nbsp;&nbsp; Select All Quarter</th>
              				
              		</tr>
              		<tr>
              				<td align="left"><input type="checkbox" id="qurtr" name="qurtr" value='1' onchange="checkAllCheckboxes()" <c:out value ="${q1status}"/> > &nbsp;&nbsp; First Quarter</td>
              		</tr>
              		<tr>
              				<td align="left"><input type="checkbox" id="qurtr" name="qurtr" value='2' onchange="checkAllCheckboxes()" <c:out value ="${q2status}"/> > &nbsp;&nbsp; Second Quarter</td>
              		</tr>
              		<tr>
              				<td align="left"><input type="checkbox" id="qurtr" name="qurtr" value='3' onchange="checkAllCheckboxes()" <c:out value ="${q3status}"/> > &nbsp;&nbsp; Third Quarter</td>
              		</tr>
              		<tr>
              				<td align="left"><input type="checkbox" id="qurtr" name="qurtr" value='4' onchange="checkAllCheckboxes()" <c:out value ="${q4status}"/> > &nbsp;&nbsp; Forth Quarter</td>
              		</tr>
              		</c:if>
								<c:if test="${projlistUnfreezeBaselSize==0}">
